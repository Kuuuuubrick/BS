...
def train(args, env_name, seed, number=1):
    ...
    list_file_graph = ['./env/g_27']
    use_gpu = torch.cuda.is_available()
    em = EnvManager(list_file_graph=list_file_graph, use_gpu=use_gpu)
    env = em.list_env[0]
    # env = gym.make(env_name)
    env_evaluate = copy.copy(env)  # When evaluating the policy, we need to rebuild an environment
    ...
    while total_steps < args.max_train_steps:
        ...
        while not done:
            ...
            if time_step % log_freq == 0:
                ...
                log_avg_reward = log_running_reward / (log_running_episodes+1)
